...
def show_student(student:data.Student):
    print(f"\nName: {student.name}\nSection: {student.section}\nSpanish Grade: {student.spanish}\tEnglish Grade: {student.english}\tSocial Studies Grade: {student.social_std}\tScience Grade: {student.science}")
...
def tops_three_average (students_list: list[data.Student]):
    students_temporal_list:list[(data.Student,float)] = actions.calculate_averages(students_list)[:3]
    print("Top students:\n")
...
def show_all_average (students_list: list[data.Student]):
    students_temporal_list:list[(data.Student,float)] = actions.calculate_averages(students_list)
    for student_tuple in students_temporal_list:
            show_student(student_tuple[0])
            print(f"Total Average grade: {student_tuple[1]}")
    input("\nPress Enter to continue")
...
def show_failed_students(students_list: list[data.Student]):
